...
public class SparkService {
  ...
  private static SubmitSparkJobRequest getSparkSubmitRequestBody(
    ...
    // Fix for Kubernetes: bind driver to all interfaces so executors can connect
    // The driver will use the pod IP automatically when binding to 0.0.0.0
    sparkProperties.put("spark.driver.bindAddress", "0.0.0.0");
  ...
  private Completable downscaleSpark(Tenant tenant, int actualWorkers, int expectedWorkers) {
    ...
    if (ApplicationConfigUtil.isKubernetesSparkCluster(config)) {
      KubernetesClient kubernetesClient = KubernetesFactory.getSparkClient(tenant);
      if (kubernetesClient != null) {
        String deploymentName = config.getSpark().getCluster().getKubernetes().getDeploymentName();
        String namespace = config.getSpark().getCluster().getKubernetes().getNamespace();
        return kubernetesClient.scaleDeployment(deploymentName, namespace, expectedWorkers);
      }
      log.error("KubernetesClient is null for tenant: {}", tenant.getValue());
      return Completable.complete();
    }

    // AWS ASG scaling logic
  ...
  private Completable upscaleSpark(Tenant tenant, int expectedWorker) {
    ...
    if (ApplicationConfigUtil.isKubernetesSparkCluster(config)) {
      ...
      if (kubernetesClient != null) {
        String deploymentName = config.getSpark().getCluster().getKubernetes().getDeploymentName();
        String namespace = config.getSpark().getCluster().getKubernetes().getNamespace();
        return kubernetesClient.scaleDeployment(deploymentName, namespace, expectedWorker);
      }
      log.error("KubernetesClient is null for tenant: {}", tenant.getValue());
      return Completable.complete();
    }
